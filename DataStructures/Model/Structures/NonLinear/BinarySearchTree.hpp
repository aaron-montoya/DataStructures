...
//  DataStructures
//
//  Created by Montoya, Aaron on 3/29/18.
...
void BinarySearchTree<Type> :: removeNode(BinaryTreeNode<Type> * removeMe)
{
    ...
    else
    {
        ...
        previous = current->getRightNode();
        removeMe->setData(current->getData());
        
        if(previous == nullptr) //Removing the root
        {
        ...
        {
            current->getLeftNode()->getRootNode(removeMe);
        }
        delete current;
    }
...
void BinarySearchTree<Type> :: remove(Type getRidOfMe)
{
    ...
    else
    {
        ...
        if(current == nullptr)
        {
            cerr << "Item not found, removal unsucccessful" << endl;
        }
