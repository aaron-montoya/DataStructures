...
//  DataStructures
//
//  Created by Montoya, Aaron on 3/19/18.
...
public:
    ...
    void removeEdgeUndiected(int source, int target);
...
/*
 ...
 Java allows for all objects to be set to nuyll but C++
 does not.
*/
...
template <class Type>
void Graph<Type> :: removeEdgeUndiected(int source, int target)
{
    assert(source >= 0 && source < vertexCount && target >= 0 && target < vertexCount);
    adjacencyMatrix[source][target] = false;
